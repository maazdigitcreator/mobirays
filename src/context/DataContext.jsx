...
export const DataProvider = ({ children }) => {
  ...
  const [productWishlistTotalsStatus, setProductWishlistTotalsStatus] = useState({
    loading: true,
    error: "",
    loaded: false,
  });
  ...
  const fetchAllData = useCallback(async (force = false) => {
    ...
    if (!force) {
      ...
      if (cachedData) {
        try {
          ...
          setDynamicPages(parsed.dynamicPages || []);
          setLoading(false);
    ...
    try {
      ...
      sessionStorage.setItem(
        CACHE_KEY,
        JSON.stringify({ products, news, reviews, brands, banners, dynamicPages: pages }),
      );
    } catch (err) {
  ...
  useEffect(() => {
    ...
    wishlistService
      .getAllTotalWishlist()
      .then((totals) => {
        setProductWishlistTotals(totals);
        setProductWishlistTotalsStatus({
          loading: false,
          error: "",
          loaded: true,
        });
      ...
      .catch((err) => {
        setProductWishlistTotalsStatus({
          ...
          error:
            err?.data?.message ||
            err?.message ||
            "Failed to load wishlist totals.",
          loaded: false,
        });
      });
  ...
  const setProductWishlistTotalCount = useCallback((productId, wishlistCount) => {
    const normalizedId = Number(productId);
    setProductWishlistTotals((current) =>
      current.map((product) =>
        Number(product.id) === normalizedId
          ? { ...product, wishlist_count: Number(wishlistCount) }
          : product,
      ),
    );
  }, []);
  ...
  const value = {
    ...
    productWishlistTotals,
    productWishlistTotalsStatus,
