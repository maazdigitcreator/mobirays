                    ...
                    categories.map((category, sectionIndex) => (
                        <div key={sectionIndex} className="flex gap-1">
                            ...
                            <div className="w-[20%] flex-shrink-0 border-2 border-[#0580A5] flex items-center justify-center">
                                <span className="text-black font-semibold text-xs sm:text-xl text-center px-2 sm:break-normal hyphens-auto">
                                    {category}
                                </span>
                            </div>
                                ...
                                {groupedSpecs[category].map((specItem, specIndex) => (
                                    <div key={specIndex} className="flex gap-1">
                                        ...
                                        <div className="w-[100px] sm:w-[130px] flex-shrink-0 text-black px-2 py-1.5 flex items-center border-2 border-[#0580A5]">
                                            <span className="text-xs sm:text-base font-semibold sm:break-normal hyphens-auto">{specItem.label}</span>
                                        </div>
                                        ...
                                        <div className={`flex-1 ${getGridCols}`}>
                                            {[0, 1, 2].map((slotIndex) => {
                                                ...
                                                return (
                                                    <div
                                                        key={slotIndex}
                                                        className={`border-2 border-[#0580A5] px-3 py-0 flex items-center mt-3 h-[100%] w-full ${slotIndex === 2 ? 'hidden lg:flex' : 'flex'}`}
                                                    >
                                                        <span className="text-black text-xs sm:text-sm break-all sm:break-normal hyphens-auto">
                                                            {value}
                                                        </span>
                                                    </div>
                                                );
